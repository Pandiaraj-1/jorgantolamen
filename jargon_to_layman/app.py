...
# Dictionary mapping medical terms to layman's explanations
...
def process():
    ...
    # Tokenize the text
    tokenized_words = word_tokenize(medical_report)

    # Replace medical terms with layman's explanations
    for i, word in enumerate(tokenized_words):
        for key in medical_explanations:
            if key.lower() in word.lower():
                tokenized_words[i] = medical_explanations[key].lower()

    # Join the tokens back into sentences
    new_report = ' '.join(tokenized_words)

    # Perform auto-correction
    corrected_text = tool.correct(new_report)
